            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <section class="bg-white">
                    ...
                    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
                        ...
                        @endif
                        <h2 class="mb-4 text-xl font-bold text-gray-900 ">Edit Attendee</h2>
                            ...
                            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                                <div class="sm:col-span-2">
                                    ...
                                    <input type="text" name="name" id="name"
                                           ...
                                           placeholder="Type attendee name"  value="{{ old('name', $attendee->name) }}">
                                    @error('name')
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <input type="email" name="email" id="email"
                                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5"
                                           placeholder="Type attendee email"  value="{{ old('email', $attendee->email) }}">
                                    @error('email')
                                    <span class="text-red-500">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="w-full">
                                    ...
                                    <input type="text" name="phone" id="phone"
                                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5"
                                           placeholder="+910000000000"  value="{{ old('phone', $attendee->phone) }}">
                                    @error('phone')
                                    <span class="text-red-500">{{ $message }}</span>
                                    @enderror
                                </div>
                                    ...
                                    <select id="conference" name="conference"
                                        ...
                                        @foreach($conferences as $conference)
                                            <option value="{{ $conference->id }}"
                                                {{ $attendee->conference_id == $conference->id ? 'selected' : '' }}>
                                                {{ $conference->title }} - <span>${{ $conference->ticket_cost }}</span>
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('conference')
